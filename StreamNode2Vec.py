...
# 动态embedding
def streaming_network_embedding(matrix_adjacency):
    # 完整的图（所有节点和边都在）
    ...
    # 在初始邻接矩阵中移除100个即将到来的节点，得到t0时刻的邻接矩阵
    ...
    walks = n2v_graph_t0.simulate_walks(num_walks=10, walk_length=80)
    ...
    # output_file = r'./temp_output.emb'
    # model.wv.save_word2vec_format(output_file)
    # with open(r'temp.txt', 'w', encoding='utf-8') as fp:
    #     fp.write(str(dict_node_embedding))
    ...
    for item_node in list_remove_nodes_cp:
        ...
        # 本次到来的节点被标记为激活
        nparray_state[item_node] = 1
        # 每次被激活的节点
        list_last_actived_node = [item_node]
        ...
        for int_round in range(int_all_round):
            ...
            for item_actived_node in list_last_actived_node:
                ...
                for item_iter_node in dict_iter_node:
                    # print(item_iter_node)
                    # 判断该节点是否已经被激活，
                    # 如果被激活，则跳过；否则判断是否要被激活
                    if nparray_state[item_iter_node] == 0:
                        # print(nparray_sentiment[int(matrix_adjacency_t1[-1, item_iter_node])])
        ...
        # 对于这种情况，随机选取一个embedding的节点作为被影响的节点
    ...
    for key, word in dict_node_embedding.items():
        with open(r'./result_node_embedding_StreamNode_2_1000.txt', 'a', encoding='utf-8') as fp:
